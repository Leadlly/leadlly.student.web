...
const InitialInfoSchema = z.object({
  class: z.string({ required_error: "Please select your class!" }).min(1),

  phone: z
    .string({ required_error: "Please enter your phone number" })
    .min(10)
    .max(10),
  gender: z.string({ required_error: "Please select your gender!" }).min(1),
  ...
  studentSchedule: z
    .string({ required_error: "Please select your schedule!" })
    .min(1),
...
const StudentInitialInfoForm = () => {
  ...
  const onFormSubmit = async (data: z.infer<typeof InitialInfoSchema>) => {
    ...
    try {
      ...
      const freeTrialResponse = await getFreeTrialActive();

      dispatch(userData({ ...user, ...freeTrialResponse.user }));

      toast.success(res.message, {
        description: freeTrialResponse.message,
      });

      router.replace("/?initialSetup=true");
    } catch (error: any) {
